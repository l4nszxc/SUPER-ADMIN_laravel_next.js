...
export default function VerifyEmailDynamic({
  params,
}: {
  ...
  useEffect(() => {
    const verifyEmail = async () => {
      try {
        ...
        const verifyUrl = new URL(
          `${process.env.NEXT_PUBLIC_API_URL}/api/email/verify/${params.id}/${params.token}`
        );
        if (expires) verifyUrl.searchParams.append("expires", expires);
        if (signature) verifyUrl.searchParams.append("signature", signature);
        ...
        // Redirect to home with email verified param after 2 seconds
        setTimeout(() => {
    ...
    verifyEmail();
  }, [params.id, params.token, expires, signature, router]);
